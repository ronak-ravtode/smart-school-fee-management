...
export function ChequeReconciliation() {
  ...
  const clearMutation = useMutation({
    mutationFn: (transactionId: string) =>
      apiClient.post<{ transaction: { id: string }; ledger: { id: string; paidAmount: string; status: string } }>(
        "/transactions/clear-cheque",
        { transactionId }
      ),
  ...
  const bounceMutation = useMutation({
    ...
    onSuccess: () => {
      addToast({
        ...
        description: "The cheque has been marked as bounced.",
        variant: "success",
      });
    },
  });
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => clearMutation.mutate(cheque.transactionId)}
